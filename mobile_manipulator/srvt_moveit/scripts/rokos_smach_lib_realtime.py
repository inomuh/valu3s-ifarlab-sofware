        ...
        try:
            ...
            if not list(ud.task_input):
                ...
                if get_tasks:
                    ...
                    home_position = {'Right_Rokos': [[0.0, -0.28, 1.3, 0.0, 0.0],
...
class ManipulatorMoveState(smach.State):
    ...
    def manipulator_move_client(self, path):
        rospy.wait_for_service('manipulator_service')
        try:
            manipulator_move = rospy.ServiceProxy('manipulator_service', ManipulatorPose_ino_2)
            resp = manipulator_move(path)
            return resp.status
