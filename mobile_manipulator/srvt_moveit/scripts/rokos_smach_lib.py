...
class RokosGetTasksState(smach.State):
    ...
    def __init__(self, service_name):
        smach.State.__init__(self,  outcomes=['succeeded', 'aborted', 'other','manuel'],
...
class GeneralSelectionState(smach.State):
    ...
    def execute(self, ud):
        try:
            ui_data = 2
            # task_status_data["task_status"] = "GeneralSelectionState"
            # ui_data_publisher.publish(str(task_status_data))
    ...
    def execute(self, ud):
        try:
            ...
            while not rospy.is_shutdown():
                ...
                if abs(x - self.odom_x) < self.vel_config:
                    pose_status = True
                if pose_status:
    ...
    def execute(self, ud):
        try:
            ...
            while not rospy.is_shutdown():
                ...
                if self.stop_data == 2:
                    current_joint = self.rokos_class.group.get_current_joint_values()
                    self.rokos_class.group.set_planning_time(15)                    
                    success, get_current_plan = self.rokos_class.dynamic_go_to_pose_goal([0, -0.28, 1.3])
                    self.stop_data = 0
...
class FinishState(smach.State):
    ...
    """
    def __init__(self, rokos_class):
        smach.State.__init__(self,  outcomes=['succeeded', 'aborted','task_state'])
        self.toggle_data = None
        self.rate = rospy.Rate(1)
        rospy.Subscriber("toggle", Int8, self.toggle_callback)
    ...
    def execute(self, ud):
        try:
            home_position = [0.0, 0.0, 0.9]
            success, get_current_plan = self.rokos_class.dynamic_go_to_pose_goal(home_position)
            task_status_data["task_status"] = "Finishing Task"
            ui_data_publisher.publish(str(task_status_data))
